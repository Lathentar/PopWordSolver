...
def find_words_at_pos(wordlist, board, boardSize, pos, word):
   foundWords = []

   if (pos[0] < 0 or boardSize[0] <= pos[0]):
      return foundWords

   if (pos[1] < 0 or boardSize[1] <= pos[1]):
      return foundWords

   charAtPos = board[pos[1]][pos[0]]
   ...
   if ( len(wordlist) > 1 ):
      foundWords = foundWords + find_words_at_pos(wordlist[charAtPos], newBoard, boardSize, (pos[0]-1, pos[1]-1), word)
      foundWords = foundWords + find_words_at_pos(wordlist[charAtPos], newBoard, boardSize, (pos[0]  , pos[1]-1), word)
      foundWords = foundWords + find_words_at_pos(wordlist[charAtPos], newBoard, boardSize, (pos[0]+1, pos[1]-1), word)
      foundWords = foundWords + find_words_at_pos(wordlist[charAtPos], newBoard, boardSize, (pos[0]-1, pos[1]  ), word)
      foundWords = foundWords + find_words_at_pos(wordlist[charAtPos], newBoard, boardSize, (pos[0]+1, pos[1]  ), word)
      foundWords = foundWords + find_words_at_pos(wordlist[charAtPos], newBoard, boardSize, (pos[0]-1, pos[1]+1), word)
      foundWords = foundWords + find_words_at_pos(wordlist[charAtPos], newBoard, boardSize, (pos[0]  , pos[1]+1), word)
      foundWords = foundWords + find_words_at_pos(wordlist[charAtPos], newBoard, boardSize, (pos[0]+1, pos[1]+1), word)
...
def find_words_in_board(boardSize, board, wordlist):
   foundWords = []
   for r in xrange(0, boardSize[1]):
      for c in xrange(0, boardSize[0]):
         foundWords += find_words_at_pos(wordlist, board, boardSize, (c, r), '')

   foundWords.sort(key=lambda x:len(x[0]), reverse=True)
   return foundWords

def minimize_board(boardSize, board):
   newBoard = copy.deepcopy(board)
   ...
   for r in xrange(0, boardSize[1]):
      for c in reversed(xrange(0, boardSize[0])):
         if newBoard[r][c] == '-':
   ...
   for c in reversed(xrange(0, boardSize[0])):
      isValid = False
      for r in reversed(xrange(0, boardSize[1])):
         if newBoard[r][c] != '-':
            isValid = True
            break
      if isValid:
...
def is_board_complete(boardSize, board):
   for r in xrange(0, boardSize[1]):
      for c in xrange(0, boardSize[0]):
         if board[r][c] != '-':
            return False
...
def solve_board(boardSize, board, wordlist, board_mem):
   boardAsString = board_to_string(board)
   if boardAsString not in board_mem:
      foundWords = find_words_in_board(boardSize, board, wordlist)

      board_mem[boardAsString] = None

      # run through the found words, minimize the board and repeat
      for wordBoardPair in foundWords:
         newBoard = minimize_board(boardSize, wordBoardPair[1])

         if is_board_complete(boardSize, newBoard):
         ...
         solvedBoard = solve_board(boardSize, newBoard, wordlist, board_mem);
...
def solve_popwords(wordlist, inputfile, justFindBestWord):
   ...
   with open(inputfile) as f:
      boardSize = ( int( f.readline() ), int( f.readline() ) )

      for lines in f:
         row = lines.strip()
         startingBoard.append(list(row))

         if (len(row) != boardSize[0]):
            print 'Error: row', row, ' is not long enough, expected length', boardSize[0]
            sys.exit()

      if (len(startingBoard) != boardSize[1]):
         print 'Error: not enough rows in board', boardSize[1]
   ...
   if justFindBestWord == False:
      solvedBoard = solve_board(boardSize, startingBoard, wordlist, {})
      stepNum = 1
      ...
      foundBestWord = find_words_in_board(boardSize, startingBoard, wordlist)[0]
